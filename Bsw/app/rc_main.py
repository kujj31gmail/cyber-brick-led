...
async def slave_init():
    ...
    from parser import DataParser
    from ble_led_server import get_handler
    gc.collect()

    data_parser = DataParser()
    bbl_controller = BBL_Controller()
    led_handler = get_handler()
            ...
            try:
                ...
                if sim_case:
                    try:
                        ...
                        continue
                    # LED 命令拦截: {"led": "ON:1:FF0000"}
                    if 'led' in sim_case:
                        led_handler.handle(sim_case['led'])
                        logger.info(f"[LED]CMD:{sim_case['led']}")
                    else:
